...
class ReviewsSpider(scrapy.Spider):
    ...
    custom_settings = {
        ...
        'ITEM_PIPELINES': {
            'amz_spider.pipelines.AmzSellerPipeline': 300
        },
    ...
    def start_requests(self):
        ...
        for task in task_list:
            if task[0] != "":
                task = {'sellerID': task[0], "country": task[-1]}
                url = "https://www.amazon.%s/sp" % self.country_site[
                    task['country']] + "?seller=%s&th=1&psc=1&language=en_US" % task["sellerID"]
                self.headers['Referer'] = url
                self.headers['cookie'] = self.cookie_dict[task['country']]
                yield scrapy.Request(url, meta={'country': task['country'], 'sellerID': task['sellerID'],
                                                'retry_number': 0},
    ...
    def parse(self, response):
        ...
        item = {}
        item['data'] = {}
        item['data']['sellerID'] = response.meta['sellerID']
        item['data']['country'] = response.meta['country']
        ...
        try:
            # 为验证码则更新代理状态并重试
            if ((response.status in [302, 404, 503]) or (len(response.body) < 10000)):  # 异常处理
                proxy_data['fail_count'] = 18
                self.collection.hset(name="useful_proxy", key=proxy, value=json.dumps(proxy_data))
                url = response.url
                yield scrapy.Request(url, meta={'country': country, 'sellerID': sellerID,
                                                'retry_number': 0},
                                     callback=self.parse, dont_filter=True)
                return
            table_data = response.xpath('//table[@id="feedback-summary-table"]')  # 确定有无feedback表格
            item['data']['country'] = country
            if len(table_data) > 0:
                # item['data']['rating'] = float(
                #     response.xpath('//span[@class="a-icon-alt"]/text()').extract_first().split(' ')[0])
                # 近30、90、一年、开店以来好评率
                item['data']['positive_30_days'] = table_data.xpath(
                    './tr[2]/td[2]/span/text()').extract_first().replace(
                    ',', '').replace('.', '').replace('\xa0', '')
                item['data']['positive_90_days'] = table_data.xpath(
                    './tr[2]/td[3]/span/text()').extract_first().replace(
                    ',', '').replace('.', '').replace('\xa0', '')
                item['data']['positive_12_months'] = table_data.xpath(
                    './tr[2]/td[4]/span/text()').extract_first().replace(
                    ',', '').replace('.', '').replace('\xa0', '')
                item['data']['positive_lifetime'] = table_data.xpath(
                    './tr[2]/td[5]/span/text()').extract_first().replace(
                    ',', '').replace('.', '').replace('\xa0', '')
                # 近30、90、一年、开店以来中评率
                item['data']['neutral_30_days'] = table_data.xpath('./tr[3]/td[2]/span/text()').extract_first().replace(
                    ',',
                    '').replace('.', '').replace('\xa0', '')
                item['data']['neutral_90_days'] = table_data.xpath('./tr[3]/td[3]/span/text()').extract_first().replace(
                    ',',
                    '').replace('.', '').replace('\xa0', '')
                item['data']['neutral_12_months'] = table_data.xpath(
                    './tr[3]/td[4]/span/text()').extract_first().replace(
                    ',', '').replace('.', '').replace('\xa0', '')
                item['data']['neutral_lifetime'] = table_data.xpath(
                    './tr[3]/td[5]/span/text()').extract_first().replace(
                    ',', '').replace('.', '').replace('\xa0', '')
                # 近30、90、一年、开店以来差评率
                item['data']['negative_30_days'] = table_data.xpath('tr[4]/td[2]/span/text()').extract_first().replace(
                    ',',
                    '').replace('.', '').replace('\xa0', '')
                item['data']['negative_90_days'] = table_data.xpath('tr[4]/td[3]/span/text()').extract_first().replace(
                    ',',
                    '').replace('.', '').replace('\xa0', '')
                item['data']['negative_12_months'] = table_data.xpath(
                    'tr[4]/td[4]/span/text()').extract_first().replace(
                    ',', '').replace('.', '').replace('\xa0', '')
                item['data']['negative_lifetime'] = table_data.xpath('tr[4]/td[5]/span/text()').extract_first().replace(
                    ',',
                    '').replace('.', '').replace('\xa0', '')
                # 近30、90、一年、开店以来评论总数
                item['data']['count_30_days'] = int(
                    table_data.xpath('tr[5]/td[2]/span/text()').extract_first().replace(',', '').replace('.',
                                                                                                         '').replace(
                        '\xa0', ''))
                item['data']['count_90_days'] = int(
                    table_data.xpath('tr[5]/td[3]/span/text()').extract_first().replace(',', '').replace('.',
                                                                                                         '').replace(
                        '\xa0', ''))
                item['data']['count_12_months'] = int(
                    table_data.xpath('tr[5]/td[4]/span/text()').extract_first().replace(',',
                                                                                        '').replace('.', '').replace(
                        '\xa0', ''))
                item['data']['count_lifetime'] = int(
                    table_data.xpath('tr[5]/td[5]/span/text()').extract_first().replace(',',
                                                                                        '').replace('.', '').replace(
                        '\xa0', ''))
                keyArr = ['positive_30_days', 'positive_90_days', 'positive_12_months',
                          'neutral_30_days', 'neutral_90_days', 'neutral_12_months', 'neutral_lifetime',
                          'count_12_months',
                          'negative_30_days', 'negative_90_days', 'negative_12_months', 'positive_lifetime',
                          'count_30_days', 'count_90_days']
                # 目前数据只需要总数，删掉其余数据
                for key in keyArr:
                    item['data'][key] = self.item_oper(item['data'][key])
                # item['data']['positivenum_30_days'] = int(
                #     item['data']['positive_30_days'] * int(item['data']['count_30_days']))
                # item['data']['positivenum_90_days'] = int(
                #     item['data']['positive_90_days'] * int(item['data']['count_90_days']))
                # item['data']['positivenum_12_months'] = int(
                #     item['data']['positive_12_months'] * int(item['data']['count_12_months']))
                # item['data']['positivenum_lifetime'] = int(
                #     item['data']['positive_lifetime'] * int(item['data']['count_lifetime']))
                for key in keyArr:
                    del item['data'][key]
                yield item
                print(item)
                ...
                # item['data']['rating'] = 0
                # item['data']['count_30_days'] = 0
                # item['data']['count_90_days'] = 0
                # item['data']['count_12_months'] = 0
                item['data']['count_lifetime'] = 0
                item['data']['negative_lifetime'] = 0
                # print(item)
                yield item

        except Exception as e:
            print('seller_info爬虫重试')
            yield scrapy.Request(url=response.url, meta={'country': country, 'sellerID': sellerID,
                                                         'retry_number': 0},
                                 callback=self.parse, dont_filter=True)
        finally:
            self.collection.hset(name="useful_proxy", key=proxy, value=json.dumps(proxy_data))

    def item_oper(self, d):
        if d == '-':
            d = 0
        if int(d) == 0:
            d = 0
    ...
    def get_timestamp(self):
        ...
        timestr_list = now_str.split('--')
        timestr = f"{timestr_list[0]}--{timestr_list[1]}" + "--09 00:00:00"
