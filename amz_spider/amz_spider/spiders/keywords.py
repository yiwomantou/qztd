...
'''
-----------------------------------------------------------
Name : amazon_keyword_spider
Desc : 抓取指定关键词的asin数据和其他统计数据
Redis format:
    type:list
    format:{"keyword":$keyword,"type":$type}
-----------------------------------------------------------
'''
import scrapy
# import sys
...
class SpyfuSpider(scrapy.Spider):
    name = 'keywords'
    ...
    custom_settings = {
        ...
        'ITEM_PIPELINES': {
            'amz_spider.pipelines.AmzKeywordsPipeline': 300
        },
    }
    ...
    def start_requests(self):
        # 各站点网址后缀
        self.web_country = {"us": "com", 'uk': "co.uk", 'es': 'es', 'fr': 'fr', 'it': 'it', 'au': "com.au", 'ca': "ca",
                            'jp': 'co.jp', 'de': 'de'}
        self.headers = {
        ...
        for word in word_list:
            ...
            headers['origin'] = 'https://www.amazon.' + self.web_country[country]
            url = "https://www.amazon.{}/s?k={}&qid=1551418823".format(self.web_country[country], url_word)
            self.headers['referer'] = "https://www.amazon." + self.web_country[country]
            yield scrapy.Request(url=url, meta={"page": 1, "keyword": keyword,
                                                'absolute_position': 0, 'country': country, 'retries': 0,
                                                'result_num': 0},
    ...
    def parse(self, response):
        ...
        if len(response.body) < 10000 or response.status == 503:
            ...
            headers = self.headers
            yield scrapy.Request(url=response.url, meta=meta,
                                 callback=self.parse, dont_filter=True)
            ...
            results = response.xpath('//span[@data-component-type="s-search-results"]/div/div[@data-asin]')
            ...
            if len(results) == 0 and metas['retries'] < 30:
                ...
                metas['retries'] += 1
                self.headers['referer'] = response.url
                self.headers['origin'] = 'https://www.amazon.' + self.web_country[country]
                headers = self.headers
                yield scrapy.Request(url=response.url, meta=metas, callback=self.parse,
                                     dont_filter=True, )
                return
            # 获取页面上的asin数据
            ...
            for result in results:
                ...
                if asin != '':
                    ...
                    ad_str = result.xpath('./div//span/span/span/span/text()').extract_first()
                    print(ad_str, "====")
                    if ad_str in ["Sponsored", "Sponsorisé", "Sponsorizzato", "Patrocinado", "スポンサー プロダクト",
                                  "Gesponsert"]:
                        result_dict['ad'] = 1
                    else:
                        result_dict['ad'] = 0
            ...
            # 获取下一页url
            temp_url = response.xpath('//ul[@class="a-pagination"]/li[@class="a-last"]/a/@href').extract_first()

            # 只抓取前300个asin,为了确保去重后不少于300个，设置判断条件为400个
            if result_num <= 400 and temp_url is not None:
                next_url = "https://www.amazon." + self.web_country[response.meta['country']] + temp_url
                self.headers['referer'] = response.url
                self.headers['origin'] = 'https://www.amazon.' + self.web_country[country]
                headers = self.headers
                yield scrapy.Request(url=next_url,
                                     meta={"page": response.meta['page'] + 1, "keyword": keyword,
                                           # 'absolute_position': ab_position,
                                           'country': country,
                                           'retries': 0, "result_num": result_num}, callback=self.parse,
                                     dont_filter=True)
            else:
                mysql_server = Mysql_server()
                cursor = mysql_server.get_cursor()
