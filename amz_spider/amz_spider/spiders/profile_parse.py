...
class ReviewsSpider(scrapy.Spider):
    ...
    custom_settings = {
        ...
        'ITEM_PIPELINES': {
            'amz_spider.pipelines.AmzProfilePipeline': 300
        },
    ...
    def parse(self, response):
        ...
        if 5000 < len(response.body) < 10000 or response.status == 503:
            ...
            yield scrapy.Request(url=response.url, meta={'country': country, "profileID": profileID},
                                 headers=self.headers, callback=self.parse, dont_filter=True)
            return
        try:
            data = json.loads(response.body)
            helpfulVotes = data.get('helpfulVotes', {})  # 有帮助数
            if len(helpfulVotes) > 0:
                item['helpfulVotes'] = int(
                    helpfulVotes.get('helpfulVotesData', {}).get('count', "0").replace(",", '').replace(".", '').replace("\xa0", ''))
            else:
                item['helpfulVotes'] = 0
            reviews = data.get('reviews', {})
            if len(helpfulVotes) > 0:
                item['reviews'] = int(    # 评论数
                    reviews.get('reviewsCountData', {}).get('count', "0").replace(",", '').replace(".", '').replace("\xa0", ''))
            else:
                item['reviews'] = 0
            # url = 'https://www.amazon.com/profilewidget/bio/amzn1.account.AE3AW62XZGNMAIDF3KEOPIAX7PDQ?view=visitor'
            url = "https://www.amazon.%s/" % self.country_site[
                country] + "profilewidget/bio/amzn1.account.%s?view=visitor" % profileID

            yield scrapy.Request(url=url, meta={'country': country, 'profileID': profileID, "item": item},
                                 headers=self.headers, callback=self.parse_info, dont_filter=True)
        except:
            print('准备重试')
            # 解析失败重试
            yield scrapy.Request(url=response.url, meta={'country': country, 'profileID': profileID,
    ...
    def parse_info(self, response):
        ...
        if 5000 < len(response.body) < 10000 or response.status == 503:
            proxy_data['fail_count'] = 18
            self.collection.hset(name="useful_proxy", key=proxy, value=json.dumps(proxy_data))
            yield scrapy.Request(url=response.url, meta={'country': country, "profileID": profileID, "item": item},
                                 headers=self.headers, callback=self.parse_info, dont_filter=True)
            return
        try:
            data = json.loads(response.body)
            item['location'] = data.get("location", "")   # 位置
            if item['location'] == None:
                item['location'] = ""
            item['occupation'] = data.get("occupation", "")   # 职业
            if item['occupation'] == None:
                item['occupation'] = ""
            social = data.get("social", {}).get('socialLinks', [])   # 各平台主页链接
            for url_data in social:
                url_type = url_data['type']
                url = "" if url_data['url'] == None else url_data['url']
                if url_type == "facebook":
                    ...
                    item['youtube'] = url
            # 排名
            topReviewerInfo = {} if data.get("topReviewerInfo", {}) == None else data.get("topReviewerInfo", {})
            if len(topReviewerInfo) > 0:
                item["rank"] = 0 if topReviewerInfo.get("rank", 0) == None else int(topReviewerInfo.get("rank", 0))
            else:
                item["rank"] = 0
            url = "https://www.amazon.%s/" % self.country_site[country] + "gp/profile/amzn1.account.%s" % profileID

            yield scrapy.Request(url=url, meta={'country': country, 'profileID': profileID, "item": item},
                                 headers=self.headers, callback=self.parse_name, dont_filter=True)
        except:
            print('准备重试')
            yield scrapy.Request(url=response.url, meta={'country': country, 'profileID': profileID, "item": item, },
                                 headers=self.headers, callback=self.parse_info, dont_filter=True)
    ...
    def parse_name(self, response):
        ...
        proxy_data = {"proxy": proxy,
                      ...
                      "check_count": 20, "last_status": 0,
                      "last_time": ""}
        if 5000 < len(response.body) < 10000 or response.status in[503, 403]:
            proxy_data['fail_count'] = 18
            self.collection.hset(name="useful_proxy", key=proxy, value=json.dumps(proxy_data))
            ...
            return
        self.collection.hset(name="useful_proxy", key=proxy, value=json.dumps(proxy_data))
        data = re.findall('"nameHeaderData":{"name":"(.*?)"', str(response.body))
        if len(data)>0:
            item['name'] = data[0]
        else:
            item['name'] = ''
